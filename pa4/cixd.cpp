...
{
   ifstream get_in;
   get_in.open(header.filename);
   const char *get_cmd = string(string("cat ") +
                                header.filename + " 2>&1")
                             .c_str();
   FILE *get_pipe = popen(get_cmd, "r");
   if (get_in.bad() || get_pipe == NULL)
   {
   ...
   if (status < 0)
      ...
      outlog << get_cmd << ": exit " << (status >> 8)
             << " signal " << (status & 0x7F)
             << " core " << (status >> 7 & 1) << endl;

   stringstream in_data;
   while (get_in >> in_data.rdbuf())
      ;
   header.command = cix_command::FILEOUT;
   header.nbytes = in_data.str().size();
   outlog << "sending header " << header << endl;
   send_packet(client_sock, &header, sizeof header);
   send_packet(client_sock, in_data.str().c_str(),
               in_data.str().size());
   outlog << "sent " << in_data.str().size() << " bytes" << endl;
}

void reply_put(accepted_socket &client_sock, cix_header &header)
{
   ofstream put_out;
   put_out.open(header.filename);
   if (put_out.bad())
   {
      outlog << "put failed: " << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet(client_sock, &header, sizeof header);
      return;
...
{
   FILE *exists = fopen(header.filename, "r");
   if (exists == NULL)
   {
      outlog << "rm failed: " << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      send_packet(client_sock, &header, sizeof header);
      return;
   ...
   fclose(exists);
   const char *rm_cmd = string(string("unlink ") +
                               header.filename + " 2>&1")
                            .c_str();
   system(rm_cmd);
   header.command = cix_command::ACK;
