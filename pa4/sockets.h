...
protected:
   ...
   void create();
#ifdef _WIN32
   void bind(const in_addr port);
   // client_socket initialization
   void connect(const string host, const in_addr port);
#else
...
public:
#ifdef _WIN32
   client_socket(string host, in_addr port);
#else
...
public:
   int sys_errno;
   explicit socket_sys_error(const string &what) : socket_error(what + ": " + strerror(errno)),
                                                   sys_errno(errno) {}
};
...
public:
   int host_errno;
   explicit socket_h_error(const string &what) : socket_error(what + ": " + hstrerror(h_errno)),
                                                 host_errno(h_errno) {}
};
